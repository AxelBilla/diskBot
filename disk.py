...
async def returnHorn(ctx):
    file = open("serverData/" + str(ctx.guild.id) + "/" + "data.json", "r")
...
async def addHorn(ctx, txt: str):
    path = "serverData/" + str(ctx.guild.id)
    if not os.path.exists(path):
        os.makedirs(path)
        open(path+"/data.json","w")
    file = open(path + "/" + "data.json", "a")
    file.write(txt+"\n")
    await ctx.response.send_message(f"\"{txt}\" was successfully added!")
...
async def addHorn(ctx, txt: str):
    ...
    check=True
    if len(wordList)==0:
        await ctx.response.send_message("You currently do not have any saved pickup lines.")
    elif txt=="*":
        wordList=[]
        fileWrite = open("serverData/" + str(ctx.guild.id) + "/" + "data.json", "w")
        fileWrite.writelines(wordList)
        await ctx.response.send_message(f"Saved pick up lines successfully cleared!")
    else:
        for i in range(len(wordList)):
            if wordList[i]==txt+"\n" or str(i+1)==txt:
                ...
                fileWrite = open("serverData/" + str(ctx.guild.id) + "/" + "data.json", "w")
                fileWrite.writelines(wordList)
                await ctx.response.send_message(f"\"{txt}\" was successfully removed!")
                check=False
                break
        if check:
            await ctx.response.send_message(f"\"{txt}\" couldn't be found!")

@bot.tree.command(name='see_disk', description="Display all saved pickup lines.")
@app_commands.checks.has_permissions(administrator=True)
async def checkHorn(ctx):
    ...
    if len(wordList)==0:
        ...
        await ctx.response.send_message(outString)
